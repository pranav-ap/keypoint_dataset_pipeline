...
TIME_OFFSETS = {
    ...
    "MGO12_freemovement_long_session": 62613,
}
...
# DATASET_PATH = f"D:/thesis_code/datasets/{DATASET}"
...
USE_MODEL_CONFIDENCE_FOR_FILTERING = True
CONFIDENCE_THRESHOLD = 0.99  # 0.98
PATCH_SIZE = 32  # 16 32
TARGET_X, TARGET_Y = 640, 480
# TARGET_X, TARGET_Y = 960, 960  # for MI
...
class Light(pl.LightningModule):
    ...
    def forward(self, reference_patches, target_patches, estimates):
        ...
        coords_delta_pred = coords_delta_norm_pred.float() * ((patch_size - 1) / 2)
        coords_pred = estimates + coords_delta_pred
        coords_pred = torch.clamp(coords_pred, min=0.0, max=float(patch_size))
    ...
    try:
        ...
        for kpid_A, kp_A in zip(
            ...
            left_crop, left_crop_keypoint, left, upper = crop_image_alb(
                left_image, [x, y], patch_size=PATCH_SIZE
            )

            if (
                left_crop_keypoint[0] != PATCH_SIZE // 2
                or left_crop_keypoint[1] != PATCH_SIZE // 2
            ):
                continue
            ...
            right_crop, right_crop_keypoint, left, upper = crop_image_alb(
                right_image, [x, y], patch_size=PATCH_SIZE
            )
        ...
        for (
            ...
            if target_x < TARGET_X and target_y < TARGET_Y:
